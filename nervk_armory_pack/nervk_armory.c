...
/* ---------------------------------------------------------------- textures */
enum { TX_WALL, TX_FLOOR, TX_CEIL, TX_WALL2, TX_FLOOR2, TX_CEIL2, TX_GLOW, TX_COUNT };
...
static void gen_textures(void){
  ...
  /* --- tileset B: wet black pressure-vessel tissue, ribs, green biofilm --- */
  for(int y=0;y<TS;y++)for(int x=0;x<TS;x++){
    float u=x/(float)TS, v=y/(float)TS;
    float rib=fabsf(sinf((u*12.0f+fbm(u,v,3,4,1701u)*1.6f)*PI));
    float seam=(x&31)<2 || (y&63)<2;
  ...
  h2n(hh,nrm,3.8f);
  texAlb[TX_WALL2]=mktex(alb); texNrm[TX_WALL2]=mktex(nrm);

  for(int y=0;y<TS;y++)for(int x=0;x<TS;x++){
    float u=x/(float)TS, v=y/(float)TS;
    int gx=x&31, gy=y&31;
    float gr=(gx<3||gy<3)?0.0f:1.0f;
  ...
  h2n(hh,nrm,2.9f);
  texAlb[TX_FLOOR2]=mktex(alb); texNrm[TX_FLOOR2]=mktex(nrm);
  ...
  h2n(hh,nrm,3.1f);
  texAlb[TX_CEIL2]=mktex(alb); texNrm[TX_CEIL2]=mktex(nrm);
...
typedef struct { float x,z; int type; int taken; } Item;  /* 0 hp 1 bullets 2 shells 3 rockets */
static Item items[MAXITEM]; static int nitems;

typedef struct {
  float x,z,yaw,hp,flash,anim,phase,state_t,deadT;
  int state,kind;   /* state: 0 sleep 1 chase 2 windup 3 cooldown 4 dead; kind: crawler/bruiser/spitter */
} Enemy;
static Enemy en[MAXENEMY]; static int nen;

typedef struct { float x,y,z,vx,vy,vz,life; int on; } Rocket;
static Rocket rockets[MAXROCKET];
...
static void gen_level(unsigned seed){
  ...
  for(int r=1;r<nrooms;r++){
    ...
    for(int k=0;k<n && nitems<MAXITEM;k++){
      ...
      unsigned rr=xs()%10;
      it->type= rr<3?0: rr<7?1: rr<9?2:3; it->taken=0;
    }
  }
  ...
  for(int r=1;r<nrooms && nen<MAXENEMY-2;r++){
    ...
    for(int k=0;k<n && nen<MAXENEMY;k++){
      ...
      e->yaw=frand()*2*PI; e->kind=xs()%3;
      e->hp=e->kind==1?58:(e->kind==2?36:28); e->state=0; e->flash=0; e->anim=0;
...
/* ---------------------------------------------------------------- audio synth */
enum { V_SHOT, V_SGUN, V_ROCK, V_BOOM, V_TICK, V_EDIE, V_HURT, V_PICK, V_STEP, V_CLICK, V_WIN };
  ...
  for(int i=0;i<n;i++){
    ...
    for(int v=0;v<MAXVOICE;v++){
      ...
      switch(voices[v].type){
        ...
        case V_ROCK:{ float f=95.0f+70.0f*sinf(t*22); s+=saw(f*t)*expf(-t*2.0f)*0.22f;
          if(t>0.55f)voices[v].on=0; }break;
        case V_BOOM:{ float nz=arand()*expf(-t*8)*1.2f; float th=sinf(2*PI*(55.0f-t*40.0f)*t)*expf(-t*5)*1.6f;
          s+=nz+th; if(t>0.9f)voices[v].on=0; }break;
...
static void clear_runtime(void){
  ...
  for(int i=0;i<MAXROCKET;i++)rockets[i].on=0;
}
...
static void reset_game(void){
  levelNo=1;
  start_level();
  php=100; pammo=42; pshell=12; prock=4; weapon=0;
}
static void next_level(void){
  float hp=php+15, ammo=pammo+10, shells=pshell+3, rocks=prock+1;
  if(hp>100)hp=100;
  if(ammo>99)ammo=99;
  if(shells>50)shells=50;
  if(rocks>20)rocks=20;
  gseed=ihash(gseed ^ (0x9e3779b9u + (unsigned)levelNo*0x85ebca6bu));
  levelNo++;
  start_level();
  php=hp; pammo=ammo; pshell=shells; prock=rocks;
  sfx(V_WIN);
}
...
static float erad(Enemy*e){ return e->kind==1?0.78f:(e->kind==2?0.68f:0.55f); }
static void kill_enemy(Enemy*e){
  e->state=4; e->deadT=0; sfx(V_EDIE);
  spawn_parts(e->kind==1?34:24,e->x,0.6f,e->z,3.8f, e->kind==2?0.12f:0.45f, e->kind==2?0.55f:0.07f, e->kind==2?0.16f:0.05f);
  if(e->kind==2) add_templ(e->x,0.55f,e->z,5.0f,0.35f,0.4f,2.0f,0.7f);
}
static void damage_enemy(int i,float dmg,float hx,float hy,float hz){
  Enemy*e=&en[i]; if(e->state==4)return;
  e->hp-=dmg; e->flash=0.14f; if(e->state==0)e->state=1;
  spawn_parts(6,hx,hy,hz,2.4f, e->kind==2?0.08f:0.5f, e->kind==2?0.50f:0.08f, e->kind==2?0.12f:0.05f);
  sfx(V_TICK);
  if(e->hp<=0)kill_enemy(e);
}
static void explode(float x,float y,float z){
  add_templ(x,y,z,9.0f,0.28f,4.2f,2.0f,0.55f);
  spawn_parts(42,x,y,z,6.2f,1.0f,0.45f,0.12f);
  sfx(V_BOOM);
  float dxp=px-x,dzp=pz-z, dp=sqrtf(dxp*dxp+dzp*dzp);
  if(dp<3.6f){ hurt_player((3.6f-dp)*12.0f); shake=0.55f; }
  for(int i=0;i<nen;i++) if(en[i].state!=4){
    float dx=en[i].x-x,dz=en[i].z-z,d=sqrtf(dx*dx+dz*dz);
    if(d<4.2f && los(x,z,en[i].x,en[i].z)) damage_enemy(i,24.0f+(4.2f-d)*18.0f,en[i].x,0.55f,en[i].z);
  }
}
static void fire_ray(float dx,float dy,float dz,float dmg,float maxd){
  ...
  for(int i=0;i<nen;i++){
    ...
    float cx=en[i].x-ox, cy=(en[i].kind==1?1.05f:0.70f)-oy, cz=en[i].z-oz;
  ...
  if(hit>=0) damage_enemy(hit,dmg,ox+dx*th,oy+dy*th,oz+dz*th);
  else if(tw<maxd-0.1f){
    spawn_parts(4,ox+dx*tw,oy+dy*tw,oz+dz*tw,2.8f,1.0f,0.7f,0.3f);
    add_templ(ox+dx*(tw-0.1f),oy+dy*tw,oz+dz*(tw-0.1f),2.4f,0.06f,1.8f,1.2f,0.5f);
  }
}
static void fire(void){
  ...
  if(weapon==1){
    if(pshell<1){ sfx(V_CLICK); fireCD=0.25f; return; }
    pshell--; fireCD=0.82f; flashT=0.10f; kick=1.85f; sfx(V_SGUN);
    add_templ(px+dx*0.7f,EYE+dy*0.7f-0.1f,pz+dz*0.7f,6.5f,0.10f,4.0f,2.5f,1.2f);
    for(int p=0;p<9;p++){
      float sx=(frand()-0.5f)*0.13f, sy=(frand()-0.5f)*0.10f;
      float ax=dx+cosf(yr)*sx, ay=dy+sy, az=dz+sinf(yr)*sx;
      float il=1.0f/sqrtf(ax*ax+ay*ay+az*az); fire_ray(ax*il,ay*il,az*il,8.0f,22.0f);
    }
    return;
  }
  if(weapon==2){
    if(prock<1){ sfx(V_CLICK); fireCD=0.25f; return; }
    prock--; fireCD=1.05f; flashT=0.13f; kick=2.25f; sfx(V_ROCK);
    for(int i=0;i<MAXROCKET;i++) if(!rockets[i].on){
      rockets[i]=(Rocket){px+dx*0.75f,EYE+dy*0.75f-0.05f,pz+dz*0.75f,dx*13.0f,dy*13.0f,dz*13.0f,3.2f,1}; break; }
    add_templ(px+dx*0.7f,EYE+dy*0.7f-0.1f,pz+dz*0.7f,7.0f,0.12f,4.4f,2.1f,0.7f);
    return;
  }
  if(pammo<1){ sfx(V_CLICK); fireCD=0.25f; return; }
  pammo--; fireCD=0.27f; flashT=0.07f; kick=1.0f; sfx(V_SHOT);
...
static void update_rockets(float dt){
  for(int r=0;r<MAXROCKET;r++) if(rockets[r].on){
    Rocket*q=&rockets[r]; q->life-=dt;
    q->x+=q->vx*dt; q->y+=q->vy*dt; q->z+=q->vz*dt;
    q->vy-=1.1f*dt;
    add_templ(q->x,q->y,q->z,3.0f,0.035f,2.4f,1.0f,0.35f);
    spawn_parts(1,q->x,q->y,q->z,0.8f,0.9f,0.45f,0.12f);
    int boom = q->life<=0 || q->y<0.08f || solid((int)floorf(q->x/CELL),(int)floorf(q->z/CELL));
    for(int i=0;i<nen && !boom;i++) if(en[i].state!=4){
      float dx=en[i].x-q->x,dz=en[i].z-q->z,dy=(en[i].kind==1?1.0f:0.55f)-q->y;
      float rr=erad(&en[i]); if(dx*dx+dy*dy+dz*dz<rr*rr) boom=1;
    }
    if(boom){ q->on=0; explode(q->x,q->y,q->z); }
  }
}
...
static void update_enemies(float dt){
  for(int i=0;i<nen;i++){
    ...
    float spd=e->kind==1?1.75f:(e->kind==2?2.15f:3.05f);
    float ar=e->kind==2?6.6f:(e->kind==1?1.85f:1.35f);
    float wind=e->kind==1?0.55f:(e->kind==2?0.62f:0.35f);
    switch(e->state){
      case 0:
        if(d<12.5f && los(e->x,e->z,px,pz)) e->state=1;
        break;
      case 1:{
        e->yaw=atan2f(dx,-dz);
        float mx=dx*inv*spd*dt, mz=dz*inv*spd*dt;
        float ox=e->x, oz=e->z, cr=e->kind==1?0.45f:0.32f;
        move_circ(&e->x,&e->z,mx,mz,cr);
        if(fabsf(e->x-ox)+fabsf(e->z-oz) < spd*dt*0.25f){
          float a=e->yaw+ (sinf(gtime*3+e->phase)>0?1.4f:-1.4f);
          move_circ(&e->x,&e->z,sinf(a)*spd*dt,-cosf(a)*spd*dt,cr);
        }
        e->anim+=dt*(e->kind==1?5.0f:(e->kind==2?6.5f:9.0f));
      ...
      case 2:
        ...
        if(e->state_t>wind){
          if(e->kind==2){
            if(d<7.0f && los(e->x,e->z,px,pz)){ hurt_player(8); add_templ(px,EYE-0.2f,pz,4.0f,0.18f,0.3f,2.1f,0.55f); }
          } else if(d<(e->kind==1?2.1f:1.7f)) hurt_player(e->kind==1?22:11);
          e->state=3; e->state_t=0;
        } break;
      case 3:
        e->state_t+=dt;
        if(e->state_t>(e->kind==2?1.25f:(e->kind==1?1.0f:0.68f))) e->state=1;
        break;
    }
    if(e->state!=4 && d<0.75f && d>0.001f){
      e->x-= dx/d*(0.75f-d); e->z-= dz/d*(0.75f-d);
    }
  }
}
...
static void draw_enemy(Enemy*e){
  /* NERVK crawler: spend the bytes on silhouette, not polygons.
   * Still just boxes, but arranged like an angry cable-insect rather than a lunchbox. */
  ...
  float rearup = e->state==2 ? -0.75f*sinf(e->state_t/0.35f*PI) : 0.10f*sinf(e->anim*0.35f+e->phase);
  ...
  if(e->kind==1){ /* bruiser: tall pressure suit made from hate */
    ...
    for(int k=-1;k<=1;k+=2){ float eo[3]; m3v(M,k*0.12f,0.76f,-0.50f,eo); set_uM(B,e->x+eo[0],1.18f*squash,e->z+eo[2]); box_sh(0.07f,0.06f,0.03f); }
    ...
    for(int side=-1;side<=1;side+=2){
      ...
      float ao[3]; m3v(M,side*0.50f,0.25f,-0.05f,ao); set_uM(L,e->x+ao[0],0.73f*squash+ao[1]*squash,e->z+ao[2]); box_sh(0.12f,0.95f,0.12f);
      m3v(M,side*0.22f,-0.55f,0.04f,ao); set_uM(L,e->x+ao[0],0.50f*squash+ao[1]*squash,e->z+ao[2]); box_sh(0.16f,0.95f,0.16f);
    }
    return;
  }
  if(e->kind==2){ /* spitter: squat green gland with legs */
    glUniform3f(uTint,0.055f+fl*0.7f,0.18f+fl*0.18f,0.095f+fl*0.08f);
    float B[9]; memcpy(B,M,36); m3scl(B,1.0f,squash,1.0f);
    set_uM(B,e->x,0.42f*squash,e->z); box_sh(0.92f,0.62f,0.82f);
    float bo[3]; m3v(M,0,0.06f,-0.55f,bo); set_uM(B,e->x+bo[0],0.48f*squash,e->z+bo[2]); box_sh(0.58f,0.36f,0.34f);
    glUniform1f(uEmis,1); glUniform3f(uTint,0.25f,2.6f,0.75f);
    if(e->state==2) add_templ(e->x,0.58f,e->z,4.5f,0.05f,0.25f,2.2f,0.75f);
    set_uM(B,e->x+bo[0],0.53f*squash,e->z+bo[2]-0.10f); box_sh(0.26f,0.12f,0.08f);
    glUniform1f(uEmis,0); glUniform3f(uTint,0.055f,0.095f,0.065f);
    for(int li=0;li<6;li++){ int side=(li&1)?1:-1; int row=li>>1; float zrow=-0.28f+row*0.28f;
      m3rotZ(Z,side*0.92f); m3rotX(P,sinf(e->anim+row+side)*0.45f); m3mul(T,Z,P); m3mul(L,M,T);
      float hip[3]; m3v(M,side*0.42f,-0.03f,zrow,hip); set_uM(L,e->x+hip[0],0.30f*squash,e->z+hip[2]); box_sh(0.075f,0.52f,0.075f);
    }
    return;
  }

  /* abdomen: low, long, dragging its guts on the floor */
  float A[9]; memcpy(A,M,36); m3scl(A,1.0f,squash,1.0f);
  set_uM(A, e->x, 0.38f*squash+0.03f, e->z);
  box_sh(0.78f,0.30f,1.05f);

  /* raised front carapace */
  ...
  set_uM(C,e->x+o[0],0.55f*squash+o[1]*squash,e->z+o[2]);
  box_sh(0.52f,0.42f,0.42f);

  /* forward sensor block */
  m3v(M,0,0.18f,-0.86f,o);
  set_uM(C,e->x+o[0],0.58f*squash+o[1]*squash,e->z+o[2]);
  box_sh(0.34f,0.22f,0.24f);

  /* three hot eyes, deliberately too many */
  glUniform1f(uEmis,1); glUniform3f(uTint, 2.5f,0.35f,0.08f);
  for(int k=-1;k<=1;k++){
    float eo[3]; m3v(M,k*0.10f,0.23f,-1.00f,eo);
    set_uM(C,e->x+eo[0],0.62f*squash+eo[1]*squash,e->z+eo[2]);
    box_sh(0.055f,0.055f,0.025f);
  }

  /* mandibles / antennae: two little neon knives */
  glUniform3f(uTint, 1.9f,0.55f,0.10f);
  for(int s=-1;s<=1;s+=2){
    m3rotY(Z,s*0.28f); m3mul(T,M,Z);
    float mo[3]; m3v(M,s*0.13f,0.05f,-1.10f,mo);
    set_uM(T,e->x+mo[0],0.47f*squash+mo[1]*squash,e->z+mo[2]);
    box_sh(0.045f,0.045f,0.42f);
  }

  glUniform1f(uEmis,0); glUniform3f(uTint,0.075f,0.095f,0.080f);
  /* eight legs: two segment suggestion per side, with phase-skipped twitch */
  for(int li=0;li<8;li++){
    ...
    m3rotZ(Z,side*(0.78f+0.18f*sinf(e->phase+row)));
    m3rotX(P,gait);
    m3mul(T,Z,P); m3mul(L,M,T);
    float hip[3]; m3v(M,side*0.34f,-0.03f,zrow,hip);
    float knee[3]; m3v(L,0,-0.28f,0,knee);
    set_uM(L,e->x+hip[0]+knee[0],0.40f*squash+hip[1]*squash+knee[1],e->z+hip[2]+knee[2]);
    box_sh(0.055f,0.62f,0.055f);
    /* lower foot, darker and flatter */
    float foot[3]; m3v(M,side*0.76f,-0.34f,zrow+0.10f*gait,foot);
    set_uM(M,e->x+foot[0],0.12f*squash+foot[1]*0.15f,e->z+foot[2]);
    box_sh(0.28f,0.045f,0.08f);
  }

  /* trailing cable-tail segments; visible silhouette when it runs away from the light */
  glUniform3f(uTint,0.06f,0.075f,0.065f);
  for(int k=0;k<4;k++){
    float sway=0.20f*sinf(e->anim*0.7f+k+e->phase);
    m3rotY(Z,sway); m3mul(T,M,Z);
    float to[3]; m3v(M,0,0.00f,0.62f+k*0.20f,to);
    set_uM(T,e->x+to[0],0.27f*squash,e->z+to[2]);
    box_sh(0.22f-0.025f*k,0.14f,0.24f);
  }
}
static void draw_items(void){
  ...
  for(int i=0;i<nitems;i++){
    ...
    else if(items[i].type==1) glUniform3f(uTint,1.4f,1.1f,0.15f);
    else if(items[i].type==2) glUniform3f(uTint,1.9f,0.55f,0.10f);
    else                 glUniform3f(uTint,0.35f,1.8f,0.55f);
...
static void draw_world(float camx,float camy,float camz){
  ...
  int alt=levelNo&1; int texof[3]={alt?TX_WALL2:TX_WALL,alt?TX_FLOOR2:TX_FLOOR,alt?TX_CEIL2:TX_CEIL};
  for(int b=0;b<3;b++){
  ...
  for(int r=0;r<MAXROCKET;r++) if(rockets[r].on)
    billboard(rockets[r].x,rockets[r].y,rockets[r].z,0.22f,1.0f,0.45f,0.12f,0.9f,pyaw+90);
  glEnd();
...
static void draw_gun(void){
  ...
  int pc=weapon==2?5:(weapon==1?4:3);
  struct { float x,y,z,sx,sy,sz; } parts_[5]={
    {0,0.02f,-0.18f, weapon==1?0.20f:(weapon==2?0.16f:0.05f), weapon==2?0.13f:0.055f, weapon==2?0.46f:0.32f},
    {0,-0.04f,0.02f, weapon==2?0.13f:0.065f, weapon==2?0.12f:0.09f, weapon==2?0.22f:0.13f},
    {0,-0.13f,0.06f, 0.05f,0.12f,0.07f},
    {0.10f,0.02f,-0.18f,0.055f,0.055f,0.32f},
    {0,-0.005f,-0.43f,0.20f,0.20f,0.07f}};
  for(int p=0;p<pc;p++){
  ...
  if(flashT>0){
    glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA,GL_ONE);
    glEnable(GL_TEXTURE_2D); glBindTexture(GL_TEXTURE_2D,texAlb[TX_GLOW]);
    float s=(weapon==2?0.22f:(weapon==1?0.17f:0.10f))+frand()*0.05f;
...
static void draw_hud(void){
  ...
  if(gstate==ST_PLAY){
    ...
    /* weapon + ammo */
    const char*wn=weapon==0?"PISTOL":(weapon==1?"SHOTGUN":"ROCKET");
    int ca=(int)(weapon==0?pammo:(weapon==1?pshell:prock));
    char am[8]; snprintf(am,8,"%02d",ca);
    glColor4f(1,0.85f,0.3f,0.95f);
    draw_text(WINW-30-textw(am,5),WINH-66,5,am);
    glColor4f(1,1,1,0.7f); draw_text(WINW-30-textw(wn,2.2f),WINH-90,2.2f,wn);
    glColor4f(1,1,1,0.45f); draw_text(WINW-30-textw("1-2-3",1.8f),WINH-112,1.8f,"1-2-3");
    char fl[16]; snprintf(fl,16,"FLOOR-%02d",levelNo);
    ...
    if(gstate==ST_TITLE){
      ...
      draw_text((WINW-textw("WASD MOVE - MOUSE LOOK - LMB FIRE - 1-2-3 WEAPONS",2))/2,560,2,
        "WASD - MOUSE - LMB FIRE - 1-2-3 GUNS - GREEN EXIT DESCENDS");
...
int main(int argc,char**argv){
  ...
  printf("[nervk] textures synthesized in %ums\n",SDL_GetTicks()-t0);
  ...
  while(running){
    ...
    while(SDL_PollEvent(&ev)){
      if(ev.type==SDL_QUIT)running=0;
        ...
        switch(ev.key.keysym.sym){
          ...
          case '1': if(d)weapon=0; break;
          case '2': if(d)weapon=1; break;
          case '3': if(d)weapon=2; break;
    ...
    if(smoke){
      ...
      if(frame>=140){ printf("[nervk] SMOKE OK\n"); running=0; }
    }
    ...
    if(gstate==ST_PLAY){
      ...
      for(int i=0;i<nitems;i++){
        ...
        if(dx*dx+dz*dz<0.8f*0.8f){
          ...
          else if(items[i].type==1){ pammo+=14; if(pammo>99)pammo=99; }
          else if(items[i].type==2){ pshell+=5; if(pshell>50)pshell=50; }
          else { prock+=2; if(prock>20)prock=20; }
        }
      }
